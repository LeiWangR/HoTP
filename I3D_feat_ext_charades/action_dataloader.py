...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        video_seq, video_label = sample['video_seq'], sample['video_label']
    
        h_seq, w_seq = video_seq.shape[1],video_seq.shape[2]
        ch = video_seq.shape[3]
        desired_frames = video_seq.shape[0]

        if isinstance(self.output_size, int):
            # for rgb video
            if h_seq > w_seq:
                new_h_seq, new_w_seq = self.output_size * h_seq / w_seq, self.output_size
            else:
                new_h_seq, new_w_seq = self.output_size, self.output_size * w_seq / h_seq
        else:
            new_h_seq, new_w_seq = self.output_size


        new_h_seq, new_w_seq = int(new_h_seq), int(new_w_seq)
        new_video_seq=np.zeros((desired_frames,new_h_seq,new_w_seq,ch))

        for i in range(desired_frames):
            # rgb videos
            image=video_seq[i,:,:,:]
            img = transform.resize(image, (new_h_seq, new_w_seq), mode='constant')
            new_video_seq[i,:,:,:]=img
            
        return {'video_seq': new_video_seq, 'video_label': video_label}
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        h, w = video_seq.shape[1],video_seq.shape[2]
        ch = video_seq.shape[3]
        ...
        new_video_seq=np.zeros((desired_frames,new_h,new_w,ch))
        for i in range(desired_frames):
            image=video_seq[i,:,:,:]
            image = image[top: top + new_h,left: left + new_w]
            new_video_seq[i,:,:,:]=image
...
class ToTensor(object):
    def __call__(self, sample):
        video_seq, video_label = sample['video_seq'], sample['video_label']
        video_seq = video_seq.transpose((3, 0, 1, 2))
        video_seq = np.array(video_seq)
        # video_label = [video_label]
        return {'video_seq': torch.from_numpy(video_seq), 'video_label': torch.from_numpy(np.array(video_label))}


class ActionDataset(data.Dataset):
    def __init__(self, info_list, rgb_dir, opt_dir, option, transforms = None):
    ...
    def __getitem__(self, idx):
        ...
        rgb_start = self.info_list.iloc[idx, 6]
        opt_start = self.info_list.iloc[idx, 7]
        sub_N = self.info_list.iloc[idx, 8]
        stride = self.info_list.iloc[idx, 9]
        sample = self.info_list.iloc[idx, 10]
        ...
        if self.option == 'rgb':
            video_seq = self.get_rgb_video(rgb_video_path, rgb_start, sub_N, stride, sample)
        else:
            # option == 'opt'
            video_seq = self.get_opt_video(opt_x_video_path, opt_y_video_path, opt_start, sub_N, stride, sample)
        ...
        sample = {'video_seq': video_seq, 'video_label': video_label}
