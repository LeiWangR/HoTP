...
class ToTensor(object):
    def __call__(self, sample):
        video_rgb, video_opt, video_label = sample['video_rgb'], sample['video_opt'], sample['video_label']
        ...
        return {'video_rgb': torch.from_numpy(video_rgb), 'video_opt': torch.from_numpy(video_opt), 'video_label': torch.from_numpy(np.array(video_label))}


class ActionDataset(data.Dataset):
    def __init__(self, info_list, rgb_dir, opt_dir, bb_dir, mode='train', transforms = None):
    ...
    def __getitem__(self, idx):
        ...
        video_rgb = self.get_rgb_video(rgb_video_path, rgb_start, bb_dir, sub_N, stride, sample)
        video_opt = self.get_opt_video(opt_x_video_path, opt_y_video_path, opt_start, bb_dir, sub_N, stride, sample)
        ...
        sample = {'video_rgb': video_rgb, 'video_opt': video_opt, 'video_label': video_label}

        if self.transforms:
            sample = self.transforms(sample)
        return sample

    def get_rgb_video(self, rgb_video_path, rgb_start, bb_dir, sub_N, stride, sample):
    ...
    def get_opt_video(self, opt_x_video_path, opt_y_video_path, opt_start, bb_dir, sub_N, stride, sample):
        # define the number of frames needed
        desired_frames = sub_N
